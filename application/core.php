 ...
 */		
	 ...
	 * @param $is_acc			是否以数组的形式返回
	 */
	static  function getRuningRoute($is_acc=false){
	 ...
	 * 保存一个静态全局数据
	 */
	static function setData($k,$v=false,$category='STATIC_STORE'){
	 ...
	 * 获取一个静态存储数据
	 */
	static function getData($k=false, $category='STATIC_STORE', $defV=NULL){
		if (!isset($GLOBALS[V_GLOBAL_NAME][$category]) || !is_array($GLOBALS[V_GLOBAL_NAME][$category])){
			return $defV;
		}
	 ...
	 * @return 生成的URL
	 */
	static function mkModuleUrl($mRoute, $qData=false, $entry=false){
	 ...
	 * @return 还原后的值
	 */
	static function _magic_var($mixed) {
	 ...
	 * @return no nreturn
	 */
	static function M($mRoute,$param=''){
    ...
    */
	 ...
	 */
	static  function importLang($lRoute, $ext = false){
	 ...
	 * @return 模板文件路径
	 */
	static function tplFile($fRoute,$baseSkin=true){
