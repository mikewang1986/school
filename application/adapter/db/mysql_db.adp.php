...
/**************************************************
*  Created:  2010-06-08
*  LastModify: 2011-10-10
*  MysqlDB类
*
*  @Xsmart (C)2006-2099 nitInc.
*  @Author 
*
***************************************************/
...
//		}
	...
	//@@ liu 测试save时，生成的sql字符串
	function save_test($data, $id = '', $table = '', $id_name = 'id') {
	...
	/**
	 * 得到读数据库连接(使用主从数据库)时，如果不是主从架构，则返回唯一的连接(主从设置可以站点根目录中的User_config.php中设置)
	 ...
	 */
	function getConnect1($mode = 'write', $index = null, $reconnect = false) 
	{
			$p = $this->params;
		//	var_dump($p);
		//	exit;
	 ...
	 * @param $key string  将此字段值作为键值
	 * @return array|false 成功则返回二维数组
	 */
	function query($sql, $fetch_mode = MYSQL_ASSOC, $key = '') 
	{
		$log_func_start_time = microtime(TRUE);
...
//---------------------------------------   @@liu
	...
	public function list_tables() {
		...
		while($r = $this->fetch_next()) {
			...
			$tables[] = $r['Tables_in_'.strtolower(DB_NAME)];
		}
		return $tables;
	}
	...
	public function fetch_next($type=MYSQL_ASSOC) {
		if($this->state['result'])
			$res = mysql_fetch_array($this->state['result'], $type);
		//var_dump($res);
		if(!$res) {
			$this->free();
		}
	...
	public function version() {
		if(!is_resource($this->getConnect())) {
			$this->autoConnect();
		}
		return mysql_get_server_info($this->getConnect());
	}
		/**
	 * 数据库查询执行方法
	 * @param $sql 要执行的sql语句
	 * @return 查询资源句柄
	 */
	private function execute1($sql) {
		if(!is_resource($this->getConnect())) {
			$this->autoConnect();
		}
		
		$this->lastqueryid = mysql_query($sql, $this->getConnect()) or $this->halt(mysql_error(), $sql);
