...
abstract class EActiveRecord extends \yii\db\ActiveRecord {
   ...
   /* public function findByPk($pk, $condition = '', $params = array()) {
    ...
    public function getAllByIds($ids, $with = null) {
        ...
        $idsstr=arrayToCsv($ids);
        $newids='t.id in ("'.$idsstr.'")';
        //$criteria->addInCondition('t.id', $ids);
        $criteria->andWhere($newids);
        $criteria->orderBy = "FIELD(t.id," . arrayToCsv($ids) . ")";
        if (is_array($with)) {
            $criteria::with($with);
        }
        return $criteria->all();
    ...
    public function getById($id, $with = null) {
        if (is_null($id))
            ...
            return $this::getByAttributes(array('id' => $id, 'date_deleted' => null),$with);
        } else {
            return $this::getByAttributes(array('id' => $id, 'date_deleted' => null));
        }
    }
