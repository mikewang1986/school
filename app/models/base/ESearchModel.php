...
abstract class ESearchModel {
     ...
     */
    public function __construct(array $searchInputs, $with = null,$classname=null) {
       ...
       // $this->model();
        ...
        $this->buildQueryCriteria($classname);
    }
    ...
    //@Implement.
    public function buildQueryCriteria($classname) {
        $this->criteria = new \yii\db\ActiveQuery($classname);
       // $tablenamepos=strrpos($classname,"\\");
       // $tablename=substr($classname,$tablenamepos+1,strlen($classname));
        $this->criteria->from($classname::tableName(). ' as '.$this->alias);
    ...
    protected function createOutput() {
        $this->criteria->select('t.*');
        //$this->output = $this->model->findAll($this->criteria);
        $this->output=$this->criteria->all();
    }
    ...
    protected function buildCriteriaQueryOptions() {
        $this->criteria->orderBy($this->order);
